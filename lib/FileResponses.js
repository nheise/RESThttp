...
var contextUtil = require('./ContextUtil.js');
  ...
  if( self.hasRange() ) {
        ...
        fs.read( fd, buffer, 0, self.range.length, self.range.start,
          function( err, bytesRead, buffer ) {
            if( err ) {
              ...
              self.setHeader();
              self.setPartialHeader();
              contextUtil.prepare206( self.context, buffer );
              responseUtil.send206( self.context );
            }
         });
      }
    });
    ...
    fs.readFile( self.path, function( err, data ) {
      if( err ) {
        ...
        self.setHeader();
        contextUtil.prepare200( self.context, data );
        responseUtil.send200( self.context );
      }
    });
  } 
};
