...
function writeHead( httpStatusCode, reasonPhrase, context )  {
   ifCharsetSetCharset( context );
   context.responseStream.writeHead( httpStatusCode, reasonPhrase, context.response.headers );
}

function ifCharsetSetCharset( context ) {
  if( context.response.charset ) {
     context.response.headers[headerKeys.CONTENT_TYPE] = context.response.headers[headerKeys.CONTENT_TYPE] + context.response.charset;
  }
}
